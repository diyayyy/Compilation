...
int yylex()
{
	...
	while (1)
	{
		...
		else if (isdigit(t))
		{
			...
			while (isdigit(t))
			{
				yylval.dval = yylval.dval * 10 + t - '0';//将每个数字字符转换为对应的整数值，并构建整数
				t = getchar();
			}
		...
		else if ((t>='a'&&t<='z')||(t>='A'&&t<='Z')||(t=='_'))//标识符以字母下划线开头
		{
			...
			char* temp = (char*)malloc (50*sizeof(char)); //分配动态内存以存储标识符的副本。
			strcpy(temp,idStr);//复制标识符到动态分配的内存中
			yylval.symp=symlook(temp); //使用 symlook 函数查找或创建标识符的符号表项，并将结果存储在 yylval.symp 中
